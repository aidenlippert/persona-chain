...
async function handleRequest(request) {
  ...
  // Handle /rpc - this is the critical path for Keplr
  if (url.pathname === '/rpc') {
    if (request.method === 'GET') {
      // For GET requests to /rpc, return status
      targetUrl = `http://blockchain.personapass.xyz:26657/status${url.search}`
    } else if (request.method === 'POST') {
      // For POST requests to /rpc, Keplr is trying to make JSON-RPC calls
      // Since our blockchain doesn't support JSON-RPC at root, we need to parse the request
      // and route it to the correct endpoint
      ...
      try {
        ...
        if (jsonRpc.method === 'status') {
          targetUrl = `http://blockchain.personapass.xyz:26657/status`
        } else if (jsonRpc.method === 'abci_info') {
          targetUrl = `http://blockchain.personapass.xyz:26657/abci_info`
        } else if (jsonRpc.method === 'genesis') {
          targetUrl = `http://blockchain.personapass.xyz:26657/genesis`
        } else if (jsonRpc.method === 'health') {
          targetUrl = `http://blockchain.personapass.xyz:26657/health`
        } else {
          // For unknown methods, try to forward to the method endpoint
          targetUrl = `http://blockchain.personapass.xyz:26657/${jsonRpc.method}`
        }
...
REST: https://person.aidenlippert.workers.dev/api
  ...
  // For GET requests and other methods, make the standard request
  if (targetUrl) {
    try {
      const response = await fetch(targetUrl, {
