...
export class CommitmentService {
  ...
  /**
   * Create a cryptographic commitment for sensitive data
   */
  createCommitment(data: CommitmentData): Commitment {
    try {
      ...
      console.log(`🔒 Created commitment for ${data.type}:`, {
        type: data.type,
  ...
  /**
   * Create a Merkle tree commitment for multiple values
   */
  createMerkleCommitment(dataArray: CommitmentData[]): {
    ...
    try {
      ...
      console.log(`🌳 Created Merkle commitment with ${leaves.length} leaves`);
      ...
      console.error('❌ Error creating Merkle commitment:', error);
      throw new Error('Failed to create Merkle commitment');
    }
  }
  ...
  /**
   * Extract public (non-sensitive) inputs from data
   */
  private extractPublicInput(data: CommitmentData): string {
    ...
    // Add type-specific public inputs
    switch (data.type) {
      case 'degree':
        publicData.degreeType = data.degreeType;
        publicData.graduationYear = data.graduationDate?.split('-')[0];
        publicData.verified = data.verified;
        break;
        
      case 'gpa':
        publicData.scale = data.scale;
        publicData.verified = data.verified;
        // Note: actual GPA value is kept private
        break;
        
      case 'transcript':
        publicData.courseCount = data.courses?.length || 0;
        publicData.verified = data.verified;
        // Note: actual courses and grades are kept private
        break;
