#!/bin/bash
# PersonaChain Docker Startup Script
# Handles initialization and startup for Kubernetes deployment

set -e

CHAIN_ID=${CHAIN_ID:-persona-mainnet-1}
MONIKER=${MONIKER:-persona-gcp-validator}
HOME_DIR="/home/persona/.persona-chain"
...
if [ ! -f "$HOME_DIR/config/genesis.json" ]; then
    ...
    persona-chaind init "$MONIKER" --chain-id="$CHAIN_ID" --home="$HOME_DIR"
...
EOF
    ...
    if [ ! -f "$HOME_DIR/config/priv_validator_key.json" ]; then
        ...
        persona-chaind tendermint show-validator --home="$HOME_DIR"
    fi
